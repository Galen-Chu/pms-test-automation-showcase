...
class TransportServicesComponent(BasePage):
    ...
    def set_transport_service(self, fee=None, transport_type=None):
        if transport_type:
            transport_status_locator = self.formator_locator(self.locator.transport_field_combobox_text, '接/送')
            self.click(transport_status_locator)
            transport_option_locator = self.formator_locator(self.locator.transport_type_option, transport_type)
    ...
    def get_transport_grid_cell_text_by_row(self, row_index, column_index):
        """取得接送服務表格指定行和列的文字"""
        locator = self.formator_locator(self.locator.transport_data_colume_by_row_and_index, (row_index, column_index))
        return self.driver.find_element(*locator).text
