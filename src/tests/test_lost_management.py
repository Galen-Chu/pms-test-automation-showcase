...
class TestLostManagement:
    ...
    def test_add_lost_item(self):
        ...
        with allure.step("And 驗證失物資料"):
            ...
            web.lost_management_page.assert_data("遺失日期", web.lost_management_page.get_lost_item_data_from_dialog("lostDate"), "2025/06/18")
            web.lost_management_page.assert_data("物品名稱", web.lost_management_page.get_lost_item_data_from_dialog("item"), lost_item_name)
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_dialog("status"), "遺失")

    @allure.story("單筆 - 從編輯失物視窗新增資料")
    @pytest.mark.xdist_group("lost_item")
    @pytest.mark.dependency(name='test_add_item_from_dialog', depends=['test_add_lost_item'], scope='session')
    @pytest.mark.parametrize("status, date, date_field", [("遺失", "遺失日期", "lostDate"), ("拾獲", "拾獲日期", "foundDate")])
    def test_add_item_from_dialog(self, status, date, date_field):
        ...
        with allure.step("And 驗證失物資料"):
            ...
            web.lost_management_page.assert_data(date, web.lost_management_page.get_lost_item_data_from_page(date_field), today)
            web.lost_management_page.assert_data("物品名稱", web.lost_management_page.get_lost_item_data_from_page("item"), item_name)
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), status)

    @allure.story("單筆 - 遺失變成遺失已尋獲")
    @pytest.mark.xdist_group("lost_item")
    @pytest.mark.dependency(name='test_change_to_lost_and_found', depends=['test_add_item_from_dialog'], scope='session')
    def test_change_to_lost_and_found(self):
        ...
        with allure.step("And 驗證變更狀態成功"):
            ...
            today = datetime.now().strftime("%Y/%m/%d")
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "遺失已尋獲")
            web.lost_management_page.assert_data("拾獲者", web.lost_management_page.get_lost_item_data_from_page("picker"), "Jimmy")
            web.lost_management_page.assert_data("拾獲日期", web.lost_management_page.get_lost_item_data_from_page("foundDate"), today)

    @allure.story("單筆 - 遺失已尋獲變成領回")
    @pytest.mark.xdist_group("lost_item")
    @pytest.mark.dependency(name='test_lost_and_found_change_to_claim', depends=['test_change_to_lost_and_found'], scope='session')
    def test_lost_and_found_change_to_claim(self):
        ...
        with allure.step("And 驗證變更狀態成功"):
            ...
            today = datetime.now().strftime("%Y/%m/%d")
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "領回")
            web.lost_management_page.assert_data("領回者", web.lost_management_page.get_lost_item_data_from_page("recipient"), "Bark")
            web.lost_management_page.assert_data("領回日期", web.lost_management_page.get_lost_item_data_from_page("claimDate"), today)

    @allure.story("單筆 - 領回變成拾獲")
    @pytest.mark.xdist_group("lost_item")
    @pytest.mark.dependency(name='test_claim_change_to_found', depends=['test_lost_and_found_change_to_claim'], scope='session')
    def test_claim_change_to_found(self):
        ...
        with allure.step("And 驗證變更狀態成功"):
            ...
            web.base_page.screenshot("And 驗證變更狀態成功")
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "拾獲")
            web.lost_management_page.assert_data("領回者被清空", web.lost_management_page.get_lost_item_data_from_page("recipient"), "")
            web.lost_management_page.assert_data("領回日期被清空", web.lost_management_page.get_lost_item_data_from_page("claimDate"), "")
            web.lost_management_page.assert_data("拾獲者保持原樣", web.lost_management_page.get_lost_item_data_from_page("picker"), picker)
            web.lost_management_page.assert_data("拾獲日期保持原樣", web.lost_management_page.get_lost_item_data_from_page("foundDate"), found_date)
            web.lost_management_page.assert_data("領回變成拾獲時，遺失日期可編輯", lost_date_enabled, True)
            web.lost_management_page.assert_data("領回變成拾獲時，遺失者可編輯", loser_name_enabled, True)
            web.lost_management_page.assert_data("領回變成拾獲時，遺失備註可編輯", lost_remark_enabled, True)

    @allure.story("多筆 - 拾獲變更為拾獲者領回")
    @pytest.mark.xdist_group("lost_item")
    @pytest.mark.dependency(name='test_change_to_picker_claim', depends=['test_claim_change_to_found'], scope='session')
    def test_change_to_picker_claim(self):
        ...
        with allure.step("勾選多筆資料列"):
            ...
            for row in range(0, 2):
                ...
                lost_item_codes.append(web.lost_management_page.get_lost_item_data_from_page("code", row))
        ...
        with allure.step("And 驗證失物資料"):
            for code in lost_item_codes:
                ...
                today = datetime.now().strftime("%Y/%m/%d")
                web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "領回")
                web.lost_management_page.assert_data("領回者", web.lost_management_page.get_lost_item_data_from_page("recipient"),
                                                                web.lost_management_page.get_lost_item_data_from_page("picker"))
                web.lost_management_page.assert_data("領回日期", web.lost_management_page.get_lost_item_data_from_page("claimDate"), today)

                web.lost_management_page.click_first_row()
                web.base_page.click_toolbar_with_icon("edit").sleep(2)
                web.lost_management_page.assert_data("領回備註", web.lost_management_page.get_field_value_in_dialog("領回備註"), claim_note)
                web.share_panel_component.close_panel("編輯失物")
    ...
    def test_add_found_item(self):
        ...
        with allure.step("And 驗證失物資料"):
            ...
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "拾獲")
            web.lost_management_page.assert_data("拾獲者", web.lost_management_page.get_lost_item_data_from_page("picker"), "Jimmy")
            web.lost_management_page.assert_data("拾獲日期", web.lost_management_page.get_lost_item_data_from_page("foundDate"), "2025/06/18")
            web.lost_management_page.assert_data("物品名稱", web.lost_management_page.get_lost_item_data_from_page("item"), found_item_name)

    @allure.story("單筆 - 拾獲變成領回")
    @pytest.mark.xdist_group("found_item")
    @pytest.mark.dependency(name='test_found_change_to_claim', depends=['test_add_found_item'], scope='session')
    ...
    @allure.story("單筆 - 刪除")
    @pytest.mark.xdist_group("found_item")
    @pytest.mark.dependency(name='test_delete_lost_item', depends=['test_found_change_to_claim'], scope='session')
    def test_delete_lost_item(self):
        ...
        with allure.step("Then 顯示[狀態]為<刪除>"):
            ...
            web.lost_management_page.assert_data("狀態", web.lost_management_page.get_lost_item_data_from_page("statusDisplay"), "刪除")
        ...
        with allure.step("And 驗證失物資料"):
            ...
            web.base_page.assert_data("照片功能不可使用", web.tip_component.get_tip_text(), "領回和刪除不可執行照片上傳")
            web.tip_component.click_ok()
            web.base_page.assert_data("刪除功能不可使用", web.lost_management_page.item_enabled("remove"), False)
            web.base_page.assert_data("儲存功能不可使用", web.lost_management_page.item_enabled("save"), False)

    @allure.story("查詢日期預設要帶滾房租日")
    def test_rent_day_in_search(self):
        pages = [LostManagementPage, HeaderComponent, TipComponent, SharePanelComponent, BasePage, ReservationPage]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
