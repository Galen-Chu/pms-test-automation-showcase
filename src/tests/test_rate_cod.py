...
from tests.dymamic_steps.rate_cod_step import CMPRateCodStep, DRRateCodStep, \
    DURateCodStep, GRRateCodStep, HURateCodStep, MBRateCodStep, MDRateCodStep, \
    MERateCodStep, NSTRateCodStep, RTRateCodStep, SaleRateCodStep, ConRateCodStep, \
    AddRateCodStep, ComRateCodStep, FixRateCodStep, DaysRateCodStep, RateRateCodStep, \
    BSRateCodStep, DPRatRateCodStep, DPAmtRateCodStep
from tests.dymamic_steps.rate_cod_column_step import IdOnlyRateCodColumnStep, \
    DaysOverMaxRateCodColumnStep, ServiceOverMaxRateCodColumnStep, ComOverMaxRateCodColumnStep, \
    DpRatOverMaxRateCodColumnStep, DpAmtOverMaxRateCodColumnStep
...
class TestRateCod:
    ...
    @allure.story("新增房價")
    @pytest.mark.parametrize('rate_code_type',
        ['CMP', 'DR', 'DU', 'GR', 'HU', 'MB', 'MD', 'ME', 'RT',
         'Rate', 'Sale', 'Add', 'Com', 'Fix', 'Days'])
    def test_create_new_rate_cod(self, rate_code_type):
        pages = [HeaderComponent, TipComponent, SharePanelComponent, BasePage, ServiceItemComponent, RateCodDialog]
    ...
    def test_create_ratecod_not_use(self):
        ...
        with allure.step("驗證功能按鈕無法使用"):
            ...
            web.share_panel_component.assert_data('無法使用浮動房價', web.rate_cod_dialog.item_enabled('浮動房價'), False)
            web.share_panel_component.assert_data('無法使用上傳官網', web.rate_cod_dialog.item_enabled('上傳官網'), False)
            web.share_panel_component.assert_data('無法使用複製房價', web.rate_cod_dialog.item_enabled('複製房價'), False)
            web.share_panel_component.assert_data('無法使用期間一覽表', web.rate_cod_dialog.item_enabled('期間一覽表'), False)
            web.share_panel_component.assert_data('無法使用房價試算', web.rate_cod_dialog.item_enabled('房價試算'), False)


    @allure.story("新增房價-輸入欄位驗證")
    @pytest.mark.parametrize('valid_type', ['DaysOverMax', 'ServiceOverMax', 'ComOverMax', 'DpRatOverMax', 'DpAmtOverMax', 'IdOnly'])
    ...
    def test_search_rate_cod_required(self):
        ...
        with allure.step("搜尋房價-銷售中為必填"):
            web.base_page.search().sleep(2)
            web.base_page.assert_data('銷售中 為必填', web.tip_component.get_tip_text(), "銷售中 為必填")
    ...
    def test_search_rate_cod_not_found_retry(self):
        ...
        with allure.step("重新查詢房價-有資料"):
            ...
            web.base_page.search().sleep(2)
            web.base_page.assert_data_has_count('重新查詢房價-有資料', web.base_page.get_diplaying_items_count())
    ...
    def test_edit_rate_cod_special_contract(self):
        ...
        with allure.step("編輯房價-有特殊約會顯示列表"):
            ...
            web.rate_cod_dialog.assert_data('編輯房價-有特殊約會顯示列表', web.rate_cod_dialog.has_cratecod_mn("測測BB22"), True)
            web.rate_cod_dialog.assert_data('房價名稱可以更改', web.share_panel_component.get_is_enabled_by_label_3("房價名稱"), True)
            web.rate_cod_dialog.assert_data('房價群組可以更改', web.share_panel_component.get_is_enabled_by_label_3("房價群組"), True)
            web.rate_cod_dialog.assert_data('使用說明可以更改', web.share_panel_component.get_is_enabled_by_label_3("使用說明"), True)
            web.rate_cod_dialog.assert_data('銷售中可以更改', web.share_panel_component.get_is_enabled_by_checkbox("銷售中"), True)
            web.rate_cod_dialog.assert_data('合約價可以更改', web.share_panel_component.get_is_enabled_by_checkbox("合約價"), False)
            web.rate_cod_dialog.assert_data('佣金異動可以更改', web.share_panel_component.get_is_enabled_by_checkbox("佣金異動"), True)
            web.rate_cod_dialog.assert_data('房價異動可以更改', web.share_panel_component.get_is_enabled_by_checkbox("房價異動"), True)
            web.rate_cod_dialog.assert_data('網訂使用可以更改', web.share_panel_component.get_is_enabled_by_checkbox("網訂使用"), True)
            web.rate_cod_dialog.assert_data('是否加收可以更改',web.share_panel_component.get_is_enabled_by_checkbox("是否加收"), True)
            web.rate_cod_dialog.assert_data('會員積點可以更改', web.share_panel_component.get_is_enabled_by_checkbox("會員積點"), True)

        with allure.step("驗證特殊約欄位"):
            web.rate_cod_dialog.click_cratecod_mn("測測BB22").sleep(4)
            web.rate_cod_dialog.screenshot('驗證特殊約欄位')
            web.rate_cod_dialog.assert_data('房價名稱可以更改', web.share_panel_component.get_is_enabled_by_label_3("房價名稱"), False)
            web.rate_cod_dialog.assert_data('房價群組可以更改', web.share_panel_component.get_is_enabled_by_label_3("房價群組"), False)
            web.rate_cod_dialog.assert_data('使用說明可以更改', web.share_panel_component.get_is_enabled_by_label_3("使用說明"), False)
            web.rate_cod_dialog.assert_data('銷售中可以更改', web.share_panel_component.get_is_enabled_by_checkbox("銷售中"), False)
            web.rate_cod_dialog.assert_data('合約價可以更改', web.share_panel_component.get_is_enabled_by_checkbox("合約價"), False)
            web.rate_cod_dialog.assert_data('佣金異動可以更改', web.share_panel_component.get_is_enabled_by_checkbox("佣金異動"), False)
            web.rate_cod_dialog.assert_data('房價異動可以更改', web.share_panel_component.get_is_enabled_by_checkbox("房價異動"), False)
            web.rate_cod_dialog.assert_data('網訂使用可以更改', web.share_panel_component.get_is_enabled_by_checkbox("網訂使用"), False)
            web.rate_cod_dialog.assert_data('是否加收可以更改',web.share_panel_component.get_is_enabled_by_checkbox("是否加收"), False)
            web.rate_cod_dialog.assert_data('會員積點可以更改', web.share_panel_component.get_is_enabled_by_checkbox("會員積點"), False)
    ...
    def test_remove_rate_cod_not_select(self):
        ...
        with allure.step("驗證提示訊息"):
            web.base_page.assert_data('請選擇一筆資料', web.tip_component.get_tip_text(), "請選擇一筆資料")


    @allure.story("刪除房價-被其他功能使用過")
    @pytest.mark.parametrize('rate_code, tip_msg', [
    ...
    @pytest.mark.xdist_group("nst_rate_cod")
    @pytest.mark.dependency(name="test_remove_rate_cod", depends=["test_create_nst_rate_cod"], scope="session")
    ...
    @pytest.mark.xdist_group("con_rate_cod")
    @pytest.mark.dependency(name="test_contract_bind_rate_cod", depends=["test_create_con_rate_cod"], scope="session")
    def test_contract_bind_rate_cod(self, cache):
        pages = [HeaderComponent, TipComponent, CompanyPanelComponent, SharePanelComponent, BasePage]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("驗證商務公司綁定資料"):
            ...
            web.company_panel_component.assert_data('綁定合約-合約編號', web.company_panel_component.get_td_input_value('contract_cod'), '1')
            web.company_panel_component.assert_data('綁定合約-開始日期', web.company_panel_component.get_td_input_value('begin_dat'), '2024/01/17')
            web.company_panel_component.assert_data('綁定合約-結束日期', web.company_panel_component.get_td_input_value('end_dat'), '2024/01/27')
            web.company_panel_component.assert_data('綁定合約-館別', web.company_panel_component.get_td_input_value('hotel_cod'), '01 : 台北總公司及建設事業')
            web.company_panel_component.assert_data(
                '綁定合約-房價代號',
                web.company_panel_component.get_td_input_value('rate_cod'),f'{con_rate_cod[:8]} : {con_rate_cod}')
            web.company_panel_component.assert_data('綁定合約-訂房來源', web.company_panel_component.get_td_input_value('source_typ'), '01 : 簽約客戶')
            web.company_panel_component.assert_data('綁定合約-市場類別', web.company_panel_component.get_td_input_value('guest_typ'), 'FIT : 散客')
            web.share_panel_component.close_panel('商務公司維護')

    @allure.story("商務公司綁定特殊約")
    @pytest.mark.xdist_group("con_rate_cod")
    @pytest.mark.dependency(name="test_special_contract_bind_rate_cod", depends=["test_contract_bind_rate_cod"], scope="session")
    def test_special_contract_bind_rate_cod(self, cache):
        pages = [HeaderComponent, TipComponent, SharePanelComponent, CompanyPanelComponent, BasePage, RateCodDialog]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("驗證特殊約綁定成功"):
            ...
            web.rate_cod_dialog.assert_data('相關特殊約', web.rate_cod_dialog.has_cratecod_mn("BB測測"), True)
