...
class IdOnlyRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        ...
        self.web.base_page.assert_data_in_list('房價代號Normal已存在，無法重複新增', self.web.tip_component.get_tip_text(), "房價代號Normal已存在，無法重複新增")
...
class DaysOverMaxRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        self.web.rate_cod_dialog.click_data_field_id('save').sleep(2)
        self.web.base_page.assert_data('最少住宿天數需於0~999之間', self.web.tip_component.get_tip_text(), "最少住宿天數需於0~999之間")
...
class ServiceOverMaxRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        self.web.rate_cod_dialog.click_data_field_id('save').sleep(2)
        self.web.base_page.assert_data('服務費需於0~100之間', self.web.tip_component.get_tip_text(), "服務費需於0~100之間")
...
class ComOverMaxRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        self.web.rate_cod_dialog.click_data_field_id('save').sleep(2)
        self.web.base_page.assert_data('佣金需於0~100之間', self.web.tip_component.get_tip_text(), "佣金需於0~100之間")
...
class DpRatOverMaxRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        self.web.base_page.assert_data('最大值為 100', self.web.share_panel_component.get_value_by_label_3('調整數字'), "100")
...
class DpAmtOverMaxRateCodColumnStep(BaseValidColumnStep):
    ...
    def assert_result(self):
        self.web.base_page.assert_data('最大值為 99,999', self.web.share_panel_component.get_value_by_label_3('調整數字'), "99999")
