...
class TestReservation:
    ...
    def test_roomno_reservation(self, cache):
        ...
        with allure.step("驗證訂房明細"):
            ...
            web.reservation_page.assert_data("訂房公司", web.base_page.get_dropdown_input_value('acustCode'), '一般散客')
            web.reservation_page.assert_data("稱謂", web.reservation_page.get_salute_code(), 'Miss.')
            web.reservation_page.assert_data("姓名", web.reservation_page.get_guest_info('guestName'), full_name)
            web.reservation_page.assert_data("入住日期", web.reservation_page.get_guest_info('ciDate').split(' ')[0], '2024/01/05')
            web.reservation_page.assert_data("退房日期", web.reservation_page.get_guest_info('coDate').split(' ')[0], '2024/01/06')
            web.reservation_page.assert_data("房價代碼", web.reservation_page.get_guest_info('rateCode'), '現場訂房含早')
            web.reservation_page.assert_data("使用房型", web.reservation_page.get_guest_info('useCode'), 'STD: 標準客房1')
            web.reservation_page.assert_data("計價房型", web.reservation_page.get_guest_info('roomCode'), 'STD: 標準客房1')
            web.reservation_page.assert_data("單價", web.reservation_page.get_guest_info('rentAmount'), '7,900')
            web.reservation_page.assert_data("房號", web.reservation_page.get_guest_info('roomNos'), room_no)
            web.reservation_page.sleep(4)

    @allure.story("新增客房預收款")
    @pytest.mark.xdist_group("roomno_reservation")
    @pytest.mark.dependency(name="test_add_deposit", depends=["test_roomno_reservation"], scope="session")
    def test_add_deposit(self, cache):
        pages = [HeaderComponent, TipComponent, SharePanelComponent,
                GuestAccountPage, CheckinPage]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("建立客房預收款"):
            ...
            web.share_panel_component.select_by_panel('請輸入預收款方式','32:MASTER信用卡').sleep(2)
        ...
        with allure.step("驗證客房預收款"):
            ...
            web.guest_account_page.assert_data("住客姓名", web.guest_account_page.get_guest_maintain_info('姓名'), full_name)
            web.guest_account_page.assert_data("訂房公司", web.guest_account_page.get_guest_maintain_info('訂房公司'), '一般散客')
            web.guest_account_page.assert_data("小記", web.guest_account_page.get_deposit_info('item_tot'), '-7,900')
            web.guest_account_page.assert_data("帳單備註", web.guest_account_page.get_deposit_info('remark4'), f'MASTER信用卡 {room_no}-1')
            web.guest_account_page.assert_data("消費項目", web.guest_account_page.get_deposit_info('item_nam'), '9999:客房預收款')



    @allure.story("新增房務帳作業")
    @pytest.mark.xdist_group("roomno_reservation")
    @pytest.mark.dependency(name="test_new_housekeeping_account", depends=["test_add_deposit"], scope="session")
    def test_new_housekeeping_account(self, cache):
        pages = [HeaderComponent, TipComponent, SharePanelComponent,
                 HousekeepingAccountPage, GuestAccountPage]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("驗證房務帳資料"):
            ...
            web.guest_account_page.assert_data("廳別", web.guest_account_page.get_input_values('rsptCode'), 'FO:飯店櫃檯')
            web.guest_account_page.assert_data("班別", web.guest_account_page.get_input_value('shiftCode'), 'a')
            web.guest_account_page.assert_data("房號", web.guest_account_page.get_input_value('roomNos'), room_no)
            web.guest_account_page.assert_data("小分類代號_1", web.guest_account_page.get_row_item('1', 'smallType'), 'H001: Laundry')
            web.guest_account_page.assert_data("消費代號_1", web.guest_account_page.get_row_item('1', 'productNos'), 'L01: 洗衣-襯衫')
            web.guest_account_page.assert_data("小分類代號_2", web.guest_account_page.get_row_item('2', 'smallType'), 'H002: Mini Bar')
            web.guest_account_page.assert_data("消費代號_2", web.guest_account_page.get_row_item('2', 'productNos'), 'M001: 可樂')


    @allure.story("預估款維護-轉至住客帳")
    @pytest.mark.xdist_group("roomno_reservation")
    @pytest.mark.dependency(name="test_transfer_to_guest_account", depends=["test_new_housekeeping_account"], scope="session")
    def test_transfer_to_guest_account(self, cache):
        pages = [HeaderComponent, GuestAccountPage, HousekeepingAccountPage, SharePanelComponent, TipComponent]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("驗證已結帳內容"):
            ...
            web.guest_account_page.assert_data_in_list("付款方式名稱2", pay_types, 'A0: 沖客房預收款')
        ...
        with allure.step("驗證發票統一編號"):
            ...
            web.guest_account_page.assert_data("統一編號", web.guest_account_page.get_uni_cod(), '23598233')
    ...
    def test_roomtype_reservation_with_contract(self, month, day):
        pages = [BasePage, HeaderComponent, TipComponent, SharePanelComponent, ReservationPage, ReservationCardDialog]
        web = DriverHelper.create_web_browser(pages, 'pms', 'reservation/PMS0110010')
        ...
        with allure.step("And 住客查詢功能查詢驗證"):
            ...
            web.base_page.assert_data("訂房卡存在", web.reservation_page.has_target_ikey(dialog_ikey), True)
            web.base_page.assert_data("訂房公司", web.base_page.get_field_text('acust_nam'), '測測BB22')
